    ...
    public function handle()
    {
        ...
        switch ($this->variables['OS_VERSIONS']) {
            case 'debian':
                ...
                $path_apache = '/etc/apache2/sites-available/pterodactyl.conf';
        ...
        exec('unzip -q -o phpMyAdmin.zip -d . && rm phpMyAdmin.zip && mv phpMyAdmin-*/* .');
        $this->rmrfdir('phpMyAdmin-*');
        $this->info('Files installed, configuring webserver');
        if ($this->variables['WEBSERVER'] == 'nginx' && file_exists($path_nginx) == true) {
        ...
        } elseif ($this->variables['WEBSERVER'] == 'apache' && file_exists($path_apache) == true) {
            ...
            exec($restart_apache);
            $this->info('phpMyAdmin has been successfully installed. It is available on ' . env('APP_URL') . '/phpmyadmin');
        } elseif ($this->variables['WEBSERVER'] == 'nginx' && file_exists($path_nginx) == false || $this->variables['WEBSERVER'] == 'apache' && file_exists($path_apache) == false) {
        ...
        return $this->call('phpmyadmin:user:make');
    }
