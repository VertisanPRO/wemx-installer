...
class DatabaseSettingsCommand extends Command
{
    ...
    protected $description = 'Configure database settings for your application.';
    ...
    {
        $this->info('If you want to use a local connection you should be using "127.0.0.1".');
        ...
        $this->variables['DB_DATABASE'] = $this->option('database') ?? $this->ask(
            'Database Name',
            config('database.connections.mysql.database', 'wemx')
        );
        ...
        $this->variables['DB_USERNAME'] = $this->option('username') ?? $this->ask(
            'Database Username',
            config('database.connections.mysql.username', 'wemx')
        );
        ...
        try {
            ...
            if ($this->confirm('Go back and try again?')) {
                $this->database->disconnect('_wemx_command_test');
        ...
        $this->info('Database has been updated');
    ...
    {
        config()->set('database.connections._wemx_command_test', [
            'driver' => 'mysql',
        ...
        $this->database->connection('_wemx_command_test')->getPdo();
    }
}